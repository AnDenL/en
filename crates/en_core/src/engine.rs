...
pub struct EnEngine {
    ...
    pub inserters: std::collections::HashMap<
        String,
        fn(&mut bevy_ecs::world::EntityWorldMut, serde_json::Value),
    >,
}

impl EnEngine {
    pub async fn new(window: Arc<Window>, plugin_registry: crate::PluginRegistry) -> Self {
        let renderer = Renderer::new(window, RenderSettings::default())
            .await
            .unwrap();
    ...
    // Тимчасова заглушка для рендера, поки ти не переніс все в ECS
    ...
    // Тимчасова заглушка для редактора
    pub fn render_editor_view(&mut self, view: &wgpu::TextureView) {
        let mut batcher = SpriteBatcher::new();
        batcher.begin();

        // Аналогічно, тут збираємо батчі...

        self.renderer.render_to_view(&mut batcher, view);
    }
    ...
    pub fn new_headless(
        ...
        // Змінено new_for_editor на new_headless згідно з новим API Renderer
        let renderer = Renderer::new_headless(device, queue, format, RenderSettings::default());
        let mut world = World::new();
    ...
    pub fn reload_plugins(&mut self, plugin_registry: crate::PluginRegistry) {
        ...
        for template in plugin_registry.components {
            self.inserters
                .insert(template.name.to_string(), template.inserter);
            (template.register_type)(&mut registry_lock);
        }
    }
}
...
impl ApplicationHandler for EngineApp {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        // Ініціалізуємо вікно лише один раз
        if self.window.is_none() {
            ...
            self.window = Some(window.clone()); // Зберігаємо собі

            let registry = self
                .plugin_registry
                .take()
                .unwrap_or(crate::PluginRegistry {
                    components: vec![],
                    systems: vec![],
                });
            ...
            #[cfg(not(target_arch = "wasm32"))]
            {
                // Передаємо клон вікна двигуну тільки для створення поверхні (Surface)
        ...
        if let Some(engine) = self.engine.borrow_mut().as_mut() {
            match event {
                ...
                WindowEvent::RedrawRequested => {
                    engine.update();
                    if let Err(_) = engine.render() {
                        event_loop.exit();
                    }

                    // EngineApp смикає СВОЄ вікно для наступного кадру
    ...
    fn about_to_wait(&mut self, _event_loop: &ActiveEventLoop) {
        if let Some(engine) = &mut *self.engine.borrow_mut() {
            ...
            // Запитуємо перемальовку через наше вікно
            if let Some(window) = &self.window {
                window.request_redraw();
            }
        }
...
pub fn run(project_path: String, plugin_registry: crate::PluginRegistry) {
    ...
    let mut app = EngineApp {
        ...
        window: None, // Початковий стан без вікна
    };
